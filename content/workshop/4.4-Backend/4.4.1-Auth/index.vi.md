...
Backend NestJS của bạn giờ đây có một lớp xác thực JWT hoàn chỉnh được cung cấp bởi Amazon Cognito. Tất cả các endpoints API được bảo vệ bởi `JwtAuthGuard`, và các yêu cầu đã xác thực bao gồm các user claims (userId, email) được trích xuất từ JWT token. Kiến trúc này đảm bảo rằng chỉ những users được xác thực hợp lệ mới có thể truy cập các tài nguyên SpendWise API của bạn. Passport-JWT strategy xác thực các tokens sử dụng Cognito's public JWKS endpoint, và custom `@CurrentUser()` decorator cung cấp quyền truy cập thuận tiện đến thông tin user trong các controllers của bạn. Bạn hiện đã sẵn sàng triển khai Data Layer (TypeORM entities và database integration) trong phần tiếp theo.
