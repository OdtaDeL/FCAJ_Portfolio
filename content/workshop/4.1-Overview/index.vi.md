...
2. **`Application Load Balancer (ALB)`**: Phân phối lưu lượng truy cập đến backend ECS và xử lý kết thúc SSL (HTTPS).
3. **`Bastion Host`**: Cung cấp một "jump box" bảo mật cho quản trị viên để quản lý thực thể RDS trong mạng riêng.
