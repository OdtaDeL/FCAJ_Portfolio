...
Trong phần này, bạn sẽ xây dựng nền tảng mạng AWS cho SpendWise API. Điều này bao gồm tạo một Virtual Private Cloud (VPC) riêng, cấu hình 4 subnets trên 2 Availability Zones để đảm bảo tính sẵn sàng cao, thiết lập Internet Gateway để định tuyến traffic bên ngoài, và thiết lập security groups để thực thi chính sách kiểm soát truy cập mạng. Kiến trúc này đảm bảo rằng các container NestJS API của bạn vẫn an toàn và cô lập, đồng thời cho phép giao tiếp được kiểm soát với ALB và các dịch vụ bên ngoài.

## What You Will Learn

- Tạo và cấu hình VPC với DNS resolution được bật
- Thiết kế kiến trúc subnet multi-AZ (2 public, 2 private)
- Thiết lập Internet Gateway để kết nối internet
- Cấu hình public và private Route Tables
- Tạo và quản lý Security Groups cho ALB, ECS, và NAT Instance
- Hiểu biết về segmentation mạng và các nguyên tắc cô lập

## Requirements

- Tài khoản AWS với quyền tạo VPC, subnets, security groups, và route tables
- Quyền truy cập vào AWS Management Console
- Hiểu biết về CIDR notation và địa chỉ IPv4 (tùy chọn nhưng hữu ích)
- Khoảng 30-45 phút để hoàn thành phần này
- Hoàn thành [4.5 ECS Fargate Overview](../)
...
Bạn đã thành công xây dựng nền tảng mạng AWS cho SpendWise. VPC của bạn hiện chứa 4 subnets phân bố trên 2 Availability Zones với định tuyến thích hợp, quy tắc security group, và kết nối internet gateway. Kiến trúc này cung cấp tính sẵn sàng cao và cô lập bảo mật. Hạ tầng của bạn hiện đã sẵn sàng cho các dịch vụ hỗ trợ (ECR, Secrets Manager, IAM) trong phần tiếp theo.
