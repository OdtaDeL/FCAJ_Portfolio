...
﻿This guide helps you set up the AWS networking foundation for the SpendWise API: a private VPC, 4 subnets across 2 AZs, Internet Gateway, Route Tables, 3 Security Groups, and VPC endpoints for backend dependencies.
...
| :--- | :--- | :--- | :--- |
| `spendwise-api-vpc-public-alb01` | ap-southeast-2a | `10.0.1.0/24` | Public (For ALB & NAT 1) |
| `spendwise-api-vpc-public-alb02` | ap-southeast-2c | `10.0.2.0/24` | Public (For ALB & NAT 2) |
| `spendwise-api-vpc-private-ecs01` | ap-southeast-2a | `10.0.3.0/24` | Private (For ECS Tasks) |
| `spendwise-api-vpc-private-ecs02` | ap-southeast-2c | `10.0.4.0/24` | Private (For ECS Tasks) |
...
1. Create an IGW named `spendwise-api-igw`.
2. **Attach to VPC** `spendwise-api-vpc`.

### 3.2 Public Route Table
1. Create a Route Table named `spendwise-api-public-rt`.
...
- **Inbound Rule**: Allow `Custom TCP` (Port 3000) from Source: `spendwise-api-vpc-alb-sg`.

### 4.3 NAT Instance Security Group (`spendwise-api-vpc-nat-sg`)
- **Inbound Rule**: Allow **All traffic** from Source: `spendwise-api-vpc-ecs-sg`. Allow `SSH` (Port 22) from your IP.
